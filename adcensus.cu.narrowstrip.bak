...
__global__ void cross(float *x0, float *out, int size, int dim2, int dim3, int L1, float tau1)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	//printf("blockIdx.x %d\n",blockIdx.x);
	//printf("blockIdx.x %d, blockDim.x %d, threadIdx.x %d, id %d\n", blockIdx.x, blockDim.x, threadIdx.x, id);
	if (id < size) {
		int dir = id;
		int x = dir % dim3;
		dir /= dim3;
...
int cross(lua_State *L)
{
	...
	printf("in cross, THCudaTensor_nElement(state, out) return: %d\n",THCudaTensor_nElement(state, out));
	printf("in cross, THCudaTensor_size(state, out, 2) return: %d\n",THCudaTensor_size(state, out, 2));
	printf("in cross, THCudaTensor_size(state, out, 3) return: %d\n",THCudaTensor_size(state, out, 3));
	printf("L1 = %f, tau1 = %d\n",L1, tau1);
...
__global__ void sgm2(float *x0, float *x1, float *input, float *output, float *tmp, float pi1, float pi2, float tau_so, float alpha1, float sgm_q1, float sgm_q2, int direction, int size1, int size2, int size3, int step)
{
	...
	//printf("in global sgm2 d: %d\n",d);
	//printf("in global sgm2 size2: %d\n",size2);
	//printf("in global sgm2 d: %d, blockIdx.x: %d\n",d, blockIdx.x);
	output_s[d] = output_min[d] = tmp[d * size2 + blockIdx.x];
	__syncthreads();

	//printf("in global sgm2 size3: %d\n",size3);
...
int sgm2(lua_State *L)
{
	...
	printf("in sgm2 tmp size1: %d\n", THCudaTensor_size(state, tmp, 0));	
	printf("in sgm2 tmp size2: %d\n", THCudaTensor_size(state, tmp, 1));	
	printf("in sgm2 input size1: %d\n", THCudaTensor_size(state, input, 1));
	printf("in sgm2 input size2: %d\n", THCudaTensor_size(state, input, 2));
	printf("in sgm2 input size3: %d\n", THCudaTensor_size(state, input, 3));
	printf("in sgm2 output size1 = output size1 * output size3: %d\n",size1);
	printf("in sgm2 disp_max: %d\n",disp_max);
	printf("in sgm2 (size1 - 1) / disp_max + 1: %d\n",(size1 - 1) / disp_max + 1);
	printf("in sgm2 THCudaTensor_size(state, input, 2):%d\n",THCudaTensor_size(state, input, 2));
	...
	if (id < size23) {
		...
		for (int d = 0; d < size1; d++) {
			if (dim3 - d >= 0) {
				...
				for(int j = -vr; j <= vr; j++){
					float temsum = 0;
					if(id + j*size3 < size23 && id + j*size3 >= 0)
						for (int i = 0; i < size1_input; i++) {
							temsum -= L_cache[i] * input_R[i * size23 + id - d + j*size3];
						}
					if(temsum < sum)
						sum = temsum;
				}
...
int StereoJoin(lua_State *L)
{
	...
	printf("in StereoJoin: input_L size1 %d\n",THCudaTensor_size(state, input_L, 1));
	printf("in StereoJoin: output_L size1 %d\n",THCudaTensor_size(state, output_L, 1));
	printf("in StereoJoin: output_L size3 %d\n",THCudaTensor_size(state, output_L, 3));
...
{
	printf("in __global__ void remove_nonvisible\n");	
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < size) {
		int x = id % size3;
		if (y[id] >= x) {
