...
function get_window_size(net)
   ws = 1
   print("in get_window_size net.modules num: ",#net.modules)
   for i = 1,#net.modules do
      local module = net:get(i)
      print("i module.kW: ",i,module.kW)
...
if opt.a == 'train_tr' or opt.a == 'train_all' or opt.a == 'time' then
   ...
   function make_patch(src, dst, dim3, dim4, scale, phi, trans, hshear, brightness, contrast)
      ...
      print("in make_patch, src.size :",src:size())
      print("in make_patch, dst.size :",dst:size())
   ...
   for epoch = 1,14 do
      ...
      for t = 1,nnz:size(1) - opt.bs/2,opt.bs/2 do
         for i = 1,opt.bs/2 do
            ...
            print("x_batch_tr_[i *4 -3]: ",x_batch_tr_[i * 4 -3]:size())
...
function forward_free(net, input)
   ...
   for i=1,#net.modules do
      print("in forward_free: ",i,torch.typename(net.modules[i]))
   ...
   print("in forward_free net.output size: ", net.output:size())
   return currentOutput
end

function fix_border(net, vol, direction)
   local n = (get_window_size(net) - 1) / 2
   print("in fix_border %d\n",n)
   for i=1,n do
...
function stereo_predict(x_batch, id)
   ...
   if arch == 'fast' then
      ...
      vols = torch.CudaTensor(2, disp_max, x_batch:size(3), x_batch:size(4)):fill(0 / 0)
      print("net_te.output size: ", net_te.output[{{1}}]:size())
      ...
      if sm_active and opt.sm_skip ~= 'sgm' then
         ...
         do
            ...
            for _ = 1,opt.sgm_i do
               out:zero()
               print("in main vol: ",vol:size())
               print("in main out: ",out:size())
               print("in main tmp: ",tmp:size())
